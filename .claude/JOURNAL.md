...
6. **Task - Harmonize Package Names**: Changed npm package to stellars_jupyterlab_sublime_theme (unscoped) and updated theme display name to version 1.0.67<br>
   **Result**: Removed npm package scope entirely, changing from @stellars/jupyterlab_sublime_theme to stellars_jupyterlab_sublime_theme (unscoped with underscores). Updated all references across package.json, README.md, src/index.ts, pyproject.toml, **init**.py, .copier-answers.yml, .github/workflows/build.yml, and .claude/CLAUDE.md. Changed theme display name in JupyterLab menu from "Stellars Sublime" to "Stellars Sublime Dark Theme". Built version 1.0.67 successfully. Committed changes (aae0101) and tagged as STABLE_1.0.67. npm publish encountered rate limiting (429 Too Many Requests) - requires retry after cooldown period.
...
13. **Task - Toolbar Icon Spacing Refinement** (v1.0.90): Refined file browser toolbar icon spacing after reverting DirListing item padding change<br>
    **Result**: Reverted `.jp-DirListing-item` padding override from entry #12 per user feedback - file listing items stay at default 4px padding. Focused solely on toolbar icons: reduced `positioning-region` gap from 4px to 0px, added `.jp-ToolbarButtonComponent` horizontal margin override (`4px 1px`) to tighten icon spacing so all six icons fit on one row. Added launcher button-specific styling via `[data-command="launcher:create"]` selector - `max-width: 65px` (10% narrower) and `margin-right: 8px` for visual separation from other icons. Applied identical toolbar CSS overrides to the Darcula theme's `style/custom.css`. Verified all changes via Playwright screenshots against live JupyterHub instance.

14. **Task - Suppress Stuck Pressed Shadow** (v1.0.92): Killed ugly inset box-shadow that stuck on file browser Upload button after use<br>
    **Result**: JupyterLab core leaves the Upload toolbar button at `aria-pressed="true"` after a file upload completes - the button never resets, so the fastui `:host([aria-pressed="true"])` rule rendered a permanent `inset 0 0 2px 2px` shadow that looked broken. Theme can only hide the symptom, not reset the stuck state, so added scoped override `.jp-FileBrowser-toolbar jp-button[aria-pressed='true'] { box-shadow: none !important; }` to `style/custom.css` - overrides the shadow on the `<jp-button>` host without piercing shadow DOM, scoped to the file browser so genuine toggle buttons elsewhere keep their pressed feedback. Applied same override to Darcula. Updated `Makefile` to canonical v1.32 (project-local `.nodeenv/` instead of `nodeenv -p` which failed "Text file busy") and gitignored `.nodeenv/` in both themes.

15. **Task - Rename Visible Theme To GalaxaLabs Steel Dark** (v1.0.92): Changed the name shown in the JupyterLab theme menu from Stellars Sublime Dark Theme to GalaxaLabs Steel Dark Theme<br>
    **Result**: Single-line change to the `manager.register` `name` field in `src/index.ts`. **Trigger**: the theme family was rebranded under GalaxaLabs, and two light counterparts were built alongside it - `jupyterlab_galaxalabs_steel_light_theme` (this theme's light sibling) and `jupyterlab_galaxalabs_concrete_light_theme`. **Why only the display name**: the repository, the npm package `stellars_jupyterlab_sublime_theme` and the PyPI distribution keep their existing identifiers, so no installed user is broken by a package rename; only the string in Settings → Theme changes, which existing users will see rename on upgrade. No file under `ui-tests` references the old string, so the galata baselines are unaffected.

16. **Task - Drop the vendor prefix from the theme menu name** (v1.0.93): Menu entry renamed from GalaxaLabs Steel Dark Theme to Steel Dark Theme<br>
    **Result**: Single-line change to the `manager.register` `name` field in `src/index.ts`. **Trigger**: the prefix added in entry 15 read as redundant in the theme picker, where the surrounding entries carry no vendor name either. **Why the name only**: the registered string is the sole user-visible identifier, so the repository, the npm package `stellars_jupyterlab_sublime_theme` and the PyPI distribution keep their identifiers and no installed user breaks. The string is compiled into the labextension bundle, so a rename reaches users only through a release - entry 15 was committed and pushed but never built, which is why installed labs still displayed the old name until this version.

17. **Task - Rename theme menu entry to the Galaxa family pattern** (v1.0.94): Menu entry renamed from Steel Dark Theme to Galaxa Dark Theme - Steel<br>
    **Result**: Single-line change to the `manager.register` `name` field in `src/index.ts`. **Trigger**: the bare material names read badly in the theme picker, where the four siblings sorted apart from one another. The `Galaxa <mode> Theme - <material>` shape sorts all four into one contiguous block, the dark pair ahead of the light pair, and restores a family name in the form entry 16 had dropped. Package identifiers are untouched - the repository, the npm package `stellars_jupyterlab_sublime_theme` and the PyPI distribution keep their names, so no installed user breaks. No file under `ui-tests` references the string, so the galata baselines are unaffected.
